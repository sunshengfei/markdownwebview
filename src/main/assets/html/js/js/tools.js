...
//样式缓存参数
    ...
    var tmOutId;
    $.fn.commonFunction = function (msg) {
        if (tmOutId) {
            clearTimeout(tmOutId);
        }
        if (msg) {
            $('div.fred-global-alert > p').html(msg);
            $('div.fred-global-alert').removeClass('hide');
            $('div.fred-global-alert').slideDown();
        }
        if (msg) {
            tmOutId = setTimeout(function () {
                $('div.fred-global-alert').slideUp();
            }, 2000);
        }
    }
